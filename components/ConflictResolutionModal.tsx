...
const DetailRow: React.FC<{ label: string; value: React.ReactNode; isDiff: boolean }> = ({ label, value, isDiff }) => (
    <div className={`p-2 rounded-md transition-colors duration-300 ${isDiff ? 'bg-yellow-200/80' : 'bg-slate-100'}`}>
        <div className="text-xs font-semibold text-slate-500">{label}</div>
        <div className="text-sm font-medium text-slate-800 break-words">{value || <span className="italic text-slate-400">Not set</span>}</div>
    </div>
);
...
const TransactionDetailView: React.FC<{ transaction: Transaction; diffFields: Set<keyof Transaction> }> = ({ transaction, diffFields }) => {
    ...
    return (
        <div className="space-y-2">
            ...
            <DetailRow label="Total" value={<span className="font-bold text-lg text-primary-600">{formatCurrency(transaction.total)}</span>} isDiff={diffFields.has('total')} />
...
const ConflictResolutionModal: React.FC<ConflictResolutionModalProps> = ({ isOpen, conflicts, onResolve }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4" aria-modal="true" role="dialog">
            <div className="bg-slate-50 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col transform transition-all animate-[fadeIn_0.2s_ease-out_forwards] scale-95"
                ...
                <header className="p-4 border-b border-slate-200">
                    ...
                    <p className="text-center text-sm text-slate-600 mt-1">
                        The record for <strong className="text-primary-600">{local.customerName}</strong> was updated on another device. Choose which version to keep.
                    </p>
                </header>
                
                <main className="flex-1 p-4 overflow-y-auto grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {/* Local Version */}
                    <div className="border-2 border-blue-400 rounded-lg bg-white">
                        <h3 className="text-lg font-semibold text-white bg-blue-500 p-3 rounded-t-md">Your Version <span className="font-normal text-sm">(Offline)</span></h3>
                        <div className="p-3">
                        ...
                        <div className="p-3">
                            <button
                                ...
                                className="w-full py-3 bg-blue-600 text-white font-bold rounded-lg shadow-md hover:bg-blue-700 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                            >
                                Keep This Version
                            </button>
                        </div>
                    ...
                    {/* Server Version */}
                    <div className="border-2 border-green-400 rounded-lg bg-white">
                         <h3 className="text-lg font-semibold text-white bg-green-500 p-3 rounded-t-md">Server Version <span className="font-normal text-sm">(Latest)</span></h3>
                         <div className="p-3">
                         ...
                         <div className="p-3">
                             <button
                                ...
                                className="w-full py-3 bg-green-600 text-white font-bold rounded-lg shadow-md hover:bg-green-700 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                            >
                                Use Server Version
                            </button>
                         </div>
